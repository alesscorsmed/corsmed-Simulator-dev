...
kspace(1,1) = 64; %defines the size of k-space along x direction  %128x128
kspace(1,2) = 64; %defines the size of k-space along y direction
...
[pulse_sequence,isInKspace,soft_crushers,N_pulse,...
    kspace_times,BW_rf,PulseqDuration,ESP] = ...
    pulseSequenceGenerator.EPIgenerator(RFmatrix,acquiredkspace,acquiredFOV,receiverBW,...
    G_max,dt,gamma,[],[],[],halfFourierFactor);
...
while alarm(1,1)==1||alarm(1,2)==1||alarm(1,3)==1
    [elementsize,alarm] = ...
        ...
        N_pulse,dt,pulse_sequence(4:6,:),elementsize);
...
info = pulseSequenceGenerator.createInfoStruct_EPI(kspace,FOV,dt,...
    receiverBW,kspace_times,f0,sliceThickness,ESP,acquiredkspace,acquiredFOV,...
    halfFourierFactor);
...
if fast == 1
    ...
    info = pulseSequenceGenerator.createInfoStruct_EPI(kspace,FOV,dt,...
        receiverBW,kspace_times_new,f0,sliceThickness,ESP,acquiredkspace,...
        acquiredFOV,halfFourierFactor);
end
