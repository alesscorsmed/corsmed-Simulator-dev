...
pulse_sequence(5,concatenations*N_TR+1:end) = ...
    zeros(1,size(concatenations*N_TR+1:size(pulse_sequence,2),2));
...
for i=1:acquiredLines
    ...
    for iSlice=1:slices
        ...
        kspace_times(indexAcq,1) = ((i-1)*concatenations*N_TR) + ((currentConcatenation-1)*N_TR) + ...
        ...
        TR_times(indexAcq,1) = (i-1)*concatenations*N_TR + (currentConcatenation-1)*N_TR + ...
...
if mod(slices,2)
    offReson = ([1:slices]-ceil(slices/2)).*(slice_thickness+sliceGap);
else
    offReson = ([1:slices]-((slices+1)/2)).*(slice_thickness+sliceGap); 
end
