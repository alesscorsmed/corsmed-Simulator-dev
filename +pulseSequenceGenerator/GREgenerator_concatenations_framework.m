...
dt              = 10^-6;        % sec
TR              = 400*10^-3;     % sec
sliceThickness  = 0.01;        % desired slice thickness (in m)
sliceGap        = 0.01;
f0              = 64000000;     % in Hz
...
[pulse_sequence,isInKspace,pulse_sequence_TR,soft_crushers,N_pulse,N_TR,...
    ...
    [],partialFourierStruct,slices,concatenations,sliceGap);
...
% info = pulseSequenceGenerator.createInfoStruct_GRE(kspace,FOV,dt,receiverBW,TR,TE,...
...
if fast == 1
    ...
    info_new = pulseSequenceGenerator.createInfoStruct_GRE(kspace,FOV,dt,...
...
if plotON == 1
    ...
    softCrushersInd = find(soft_crushers(1,:));

    a1 = subplot(7,1,1);
    plot(timeaxis,pulse_sequence(1,:),'r')
    hold on
    plot(timeaxis(1,softCrushersInd),pulse_sequence(1,softCrushersInd),'gs')
