...
tic
model_desired_sizeofelement = [0.001,0.001,0.001];
...
[pulse_sequence,isInKspace,pulse_sequence_TR,soft_crushers,N_pulse,N_TR,...
    ...
    TEeffective,PulseqDuration,encodedkSpace] = ...
    pulseSequenceGenerator.TSEgenerator(RFmatrix1,...
    RFmatrix2,acquiredkspace,acquiredFOV,receiverBW,G_max,TR,TE,dt,gamma,ETL,[],[],[],[],...
...
%         TRmin,dt,pulse_sequence_TR(4:6,:),pulse_sequence_TR(7:8,:),...
%         elementsize);
...
info = pulseSequenceGenerator.createInfoStruct_TSE(kspace,FOV,dt,...
    receiverBW,TR,TE,kspace_times,f0,sliceThickness,orderOfKspacelines,...
