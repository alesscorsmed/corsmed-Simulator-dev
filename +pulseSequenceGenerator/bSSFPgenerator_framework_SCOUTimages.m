...
RFmatrix(3,:)   = [45];             % RF rotation angle
...
kspace(1,1) = 256; %defines the size of k-space along x direction  %128x128
kspace(1,2) = 256; %defines the size of k-space along y direction
...
[pulse_sequence,isInKspace,pulse_sequence_TR,soft_crushers,N_pulse,N_TR,...
    kspace_times,BW_rf,TR_times,receiver_phase] = ...
...
if fast == 1
    ...
    info_new = pulseSequenceGenerator.createInfoStruct_bSSFP(kspace,FOV,dt,...
        receiverBW,TR,TE,kspace_times_new,f0,sliceThickness,receiver_phase,RFmatrix);
end
