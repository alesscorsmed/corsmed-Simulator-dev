function info = createInfoStruct_GRE_conc(kspace,FOV,dt,receiverBW,TR,TE,...
    kspace_times,f0,sliceThickness,acquiredkspace,acquiredFOV,...
    encodedkSpace,RFfreqAmpl)
...
kspaceLinesTable = repmat([1:acquiredLines]',[1,kspace(1,3)]);
kspaceLinesTable = kspaceLinesTable';
info_kspace(:,6) = kspaceLinesTable(:);

kspaceSliceTable = repmat([1:kspace(1,3)]',[acquiredLines,1]);
info_kspace(:,7) = kspaceSliceTable(:);

kspaceFreqTable = repmat(RFfreqAmpl',[acquiredLines,1]);
info_kspace(:,15) = kspaceFreqTable(:);
...
info.reconstruction.ismrmrd.header.encoding.encodingLimits.kspace_encoding_step_0.center    = ...
    ((acquiredkspace(1,1)/receiverBW/dt)-1)/2;
