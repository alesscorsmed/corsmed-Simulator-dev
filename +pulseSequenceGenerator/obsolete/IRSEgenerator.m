function [pulse_sequence,isInKspace,pulse_sequence_TR,soft_crushers,N_pulse,...
    N_TR,kspace_times,BW1,TR_times,TR_actual,PulseqDuration,encodedkSpace] = ...
    IRSEgenerator(RFmatrix1,RFmatrix2,acquiredkspace,FOV,receiver_BW,G_max,TR,TE,TI,...
    IRtype,IRduration,dt,gamma,conn_localdb,experiment_id,pulseq_id,...
...
if strcmp(IRtype,'sinc')    
    [RFIR,BW,rfir_timesteps] = pulseSequenceGenerator.addRFsinc(IRduration,IRcycles,...
        IRangle,dt,gamma); %#ok
end
...
blockIR_actual_timesteps = rfir_timesteps + round(TI/dt);
blockIR_timesteps = rfir_timesteps + 1;

blockIR = zeros(8,blockIR_timesteps);
blockIR(1,1:rfir_timesteps) = RFIR(1,:);

% The phase of the IR RF pulse is -90.
blockIR(2,1:rfir_timesteps) = 90*pi/180; % phase -90 (in rads)

blockIR(7,:) = 1:size(blockIR,2);
blockIR(8,:) = [1:size(blockIR,2)-1,blockIR_actual_timesteps];
