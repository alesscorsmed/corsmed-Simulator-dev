function [pulse_sequence,isInKspace,pulse_sequence_TR,soft_crushers,N_pulse,...
    ...
    SEgenerator(RFmatrix1,RFmatrix2,acquiredkspace,FOV,receiver_BW,G_max,TR,...
...
%% BUILD TR
TRmin = size(block1,2)/2+round(TE/dt)+round(size(block4,2)*(0.5/readConjugateFactor));
...
%%
pulse_sequence_TR(6,size(block1,2)+size(block2,2)+size(block3,2)+1:...
    size(block1,2)+size(block2,2)+size(block3,2)+size(GZ2_ref,2)) = GZ2_ref;

% Add the crusher gradient right after the 180 RF pulse
pulse_sequence_TR(5,size(block1,2)+size(block2,2)+size(block3,2)+1:...
    size(block1,2)+size(block2,2)+size(block3,2)+size(GYcrusher,2)) = GYcrusher;
...
for i=1:acquiredLines
    ...
    kspace_times(i,1) = ((i-1)*(TRmin+2)) + TRmin - size(block4,2) + 1;
