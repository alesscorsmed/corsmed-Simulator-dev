function [time,signalFE,signalPE,signalSE,signalRX,areaFE,dt,effTE,rxLimits] = ...
    cartesianPhaseBalanced( receiverBW, fovFE, fovPE, fovSE,...
    numFE, numPE, numSE, ...
...
%
%	Generates a cartesian Phase-Balanced encoding (for example for SE).
%   FE:               Encoding 
...
[feTime,feSignal,areaFE,~,fePlatLimits] = ...
...
lenFeDelay  = lenPhase - lenFeRise + 1; % steps need to add to FE
lenEnc      = lenFeSignal; % total tsteps
if lenFeDelay > 0
...
fePlatLimits = fePlatLimits + lenFeDelay; 
% readout placement
