function [time,signalFE,signalPE,signalSE,signalRX,dt,effTE,rxLimits] = ...
    cartesianSemiBalanced( receiverBW, fovFE, fovPE, fovSE,...
    numFE, numPE, numSE, ...
...
%========================  CORSMED AB © 2020 ==============================
%
functionName = 'sequence.encodings.cartesianSemiBalanced';
...
% FE Fix for always hitting k0: START  SBT210406
feEchoIndex = ceil(length(feSignal)/2);
feAreaRw = dt*sum(feSignal(1:feEchoIndex));
% FE Fix for always hitting k0: END
[feRwTime,feRwSignal,~,~,~] = ...
...
lenEnc      = lenFeRw + lenFeSignal; % total tsteps
if lenFeDelay > 0
