function [time,signalFE,signalPE,signalSE,signalRX,dt,effTE,rxLimits] = ...
    multiEchoCartesianFullyBalanced( echoTimes, receiverBW, ...
...
%
% SEQUENCE.ENCODINGS.MULTIECHOCARTESIANFULLYBALANCED
%
%	Generates a cartesian Fully-Balanced encoding (for example for BSSFP).
%   FE:  -HalfRewind  +Encoding  -Rewind  +Encoding ... -HalfRewind
%   PE:   HalfRewind                                    -HalfRewind
%   SE:   HalfRewind                                    -HalfRewind
%
% INPUT
...
%========================  CORSMED AB © 2020 ==============================
%
functionName = 'sequence.encodings.multiEchoCartesianFullyBalanced';
...
% Fix for hitting k0 START
feEchoIndex = ceil(length(feSignal)/2); 
feAreaRw = dt*sum(feSignal(1:feEchoIndex));
feAreaRw2 = feArea - feAreaRw;
% Fix for hitting k0 END

[feRwTime,feRwSignal,~,~,~] = ...
    sequence.waveforms.grTrapArea(feAreaRw,maxGStrenght,gradSlewrate,dt);

% Fix for hitting k0 START
[feRw2Time,feRw2Signal,~,~,~] = ... 
    sequence.waveforms.grTrapArea(feAreaRw2,maxGStrenght,gradSlewrate,dt); % SBT210406
% Fix for hitting k0 END
...
lenFeRw2    = length(feRw2Time);
lenFeRise   = fePlatLimits(1);
lenFeSignal = length(feTime);
lenFeDelay  = lenPhase - (lenFeRise+lenFeRw); % steps need to add to FE
lenEnc      = 2*lenFeRw + lenFeSignal; % total tsteps
if lenFeDelay > 0
    % add extra time steps
    lenEnc = lenEnc + 2*lenFeDelay; 
else
    lenFeDelay = 0;
end
% correct with echoes
lenFullFeRw = length(feFullRwSignal);
lenEchoes   = (numEchoes-1)*(lenFeSignal + lenFullFeRw)  + sum(lenEchoDelay(:));
lenEnc      = lenEnc + lenEchoes;

%% combine parts into signals
% last active entry of the frequency encoding
numTimePoints   = lenEnc;
% create signals
...
signalPE(lenEnc+1 - (1:lenPe))          = -peSignal(:); % reverse from end
signalSE(1:lenSe)                       =  seSignal(:);
signalSE(lenEnc+1 - (1:lenSe))          = -seSignal(:); % reverse from end
idxShift = lenFeDelay;
...
for ii = 2:numEchoes
    % add full rewind
    signalFE( (1:lenFullFeRw) + idxShift) = -feFullRwSignal(:);
    idxShift = idxShift + lenFullFeRw + lenEchoDelay(ii-1);
    % add FE
    signalFE( (1:lenFeSignal) + idxShift) =  feSignal(:);
...
% final half rewind
signalFE( (1:lenFeRw2) + idxShift) = -feRw2Signal(:); 
% time
