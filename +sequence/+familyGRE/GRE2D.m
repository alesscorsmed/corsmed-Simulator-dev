function [pulseSequence] = GRE2D(...
    acquisition, encoding, mrSystem, expControl)
%
% SEQUENCE.FAMILYGRE.GRE2D
%
%	Generates a Spoiled 2D Gradient Echo sequence.
%
% INPUT
%   acquisition         
%   mrSystem        
...
%========================  CORSMED AB © 2020 ==============================
%
functionName = 'sequence.familyGRE.GRE2D';
...
if isfield(expControl.sequence,'rfCosTrain') && expControl.sequence.rfCosTrain
    rfEncodingLevels = rfEncodingLevels.* ...
        abs(sin(6*(1:size(rfEncodingLevels,1))*pi/180))';
end
...
if acqData.spoiled == 1
    pulseSequence.name  = 'Spoiled Gradient Echo 2D';
else
    pulseSequence.name  = 'Gradient Echo 2D';
end
pulseSequence.type      = 'GRE2D';
