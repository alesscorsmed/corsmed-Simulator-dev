function [pulseSequence] = irFISP(...
    acquisition, encoding, mrSystem, expControl)
%
% SEQUENCE.FAMILYSSFP.IRFISP
%
%	Generates a IR with FISP kernel sequence.
%
% INPUT
%   acquisition         
%   mrSystem        
...
%========================  CORSMED AB © 2020 ==============================
%
functionName = 'sequence.familySSFP.irFISP';
...
% no RF spoiling
% grad spoilers along frequency and another direction (slice?)
[repetition] = sequence.repetitions.balancedGradEchoFISP(...
...
peEncodingLevels(idxRealReps)   = repmat(encPlan.peEncodings(:),numREPS,1);
...
rfEncodingLevels(idxRealReps) = rfEncodingLevels(idxRealReps).* ...
    cos(repmat(encPlan.encPhase(:),numREPS,1));
...
pulseSequence.name         = 'IR FISP';
pulseSequence.type         = 'IR-FISP';
