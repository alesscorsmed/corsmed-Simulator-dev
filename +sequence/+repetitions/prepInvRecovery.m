...
minTI = rfIRTime(end) - (sum(rfIRTime(rfLimitsIR))/2);
...
%% assign end time and apply swc
if prepIR.postRFswc
    repetition.swcSignal(end-1,1)     	= 1;
    repetition.time(end-1,1)            = rfIRTime(end) + expControl.sequence.dtRF;
    repetition.time(end,1)              = repetition.time(end-1,1) - ...
        expControl.sequence.dtRF + tWaitIR;
else
