function [repetition] = unbalancedGradEcho(...
    acqData, mainRF, mrSystem, expControl)
%
% SEQUENCE.REPETITIONS.UNBALANCEDDGRADECHO
%
%	Generates a Gradient Echo repetition, with semibalanced encoding.
%
% INPUT
%   acqData   
%   mainRF
...
%========================  CORSMED AB © 2020 ==============================
%
functionName = 'sequence:repetitions:unbalancedGradEcho';
...
% if acqData.numEchoes > 1
%     % array with time between echoes
%     echoTimes = (1:acqData.numEchoes-1)*acqData.deltaTimeOOP;
%     % multi-Echo encoding
%     [encTime,signalFE,signalPE,signalSE,signalRX,dtEnc,effTE,rxLimits] = ...
%         sequence.encodings.multiEchoCartesianSemiBalanced(echoTimes, ...
...
%% generate semi-balanced GRE encoding
% define vars for semi-balanced encoding
freqBalance         = 'prep';
phaseBalance        = 'prep';
sliceBalance        = 'prep';
% echoes
if acqData.numEchoes > 1
    % array with time between echoes
    echoTimes = ones(acqData.numEchoes-1,1)*acqData.deltaTimeOOP;
    reverseEchoPolarity = 0;
else
    echoTimes           = [];
...
repetition.type         = 'Unbalanced Gradient Echo';
...
%% verify PE area
encTdiff = reshape( diff([0.0; encTime]),[],1 );
if abs( sum(repetition.timeDiff.*repetition.peSignal) - ...
        sum(encTdiff.*signalPE) ) > 1e-12
    ME = MException('sequence:wrongGradArea',...
        '%s : PE gradient area mismatch in Unbalanced GRE encoding',functionName);
    throw(ME);
end
%% verify FE area
if abs( sum(repetition.timeDiff.*repetition.feSignal) - ...
        sum(encTdiff.*signalFE) ) > 1e-12
    ME = MException('sequence:wrongGradArea',...
        '%s : FE gradient area mismatch in Unbalanced GRE encoding',functionName);
    throw(ME);
end
%% verify SE area
if abs( sum(repetition.timeDiff.*repetition.seSignal) - ...
        sum(encTdiff.*signalSE) ) > 1e-12
    ME = MException('sequence:wrongGradArea',...
        '%s : SE gradient area mismatch in Unbalanced GRE encoding',functionName);
    throw(ME);
end
%% verify SS area
