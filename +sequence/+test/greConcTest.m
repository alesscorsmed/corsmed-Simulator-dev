...
% define the class
classdef greConcTest < matlab.unittest.TestCase
    ...
    properties (TestParameter)
        ...
        numSlices           = {   2,        6,                  50,                 10,         25,         8,                  15};
        rxBW                = {50e3,        75e3,               100e3,              50e3,       250e3,      150e3,              200e3 };
        TR                  = {15e-3,       40e-3,              7e-3,               50e-3,      200e-3,     150e-3,             4000e-3};
        TE                  = {3.5e-3,      10e-3,              4e-3,               5e-3,       25e-3,      5e-3,               7e-3};
    ...
    methods (Test, ParameterCombination = 'sequential')
        ...
        function runGreConEdgeTest(testCase,...
                fovFE,...
                fovPE,...
                numFE,...
                numPE,...
            ...
            % fixed vars
            acquisition.data.pulseSeqFamilyName = 'GRE-Conc';
            acquisition.data.pulseSeqActualName = 'GRE-Conc';
            ...
            acquisition.data.fovSE              = sliceThickness;
            ...
            acquisition.data.concatenations     = concatenations;
        ...
        function runGreConcGeneralTest(testCase)
            ...
            for lfovFE = linspace(0.10, 0.35, 3)
            for lfovPE = linspace(0.15, 0.35, 3)
            for lnumFE = [128 256]
            for lnumPE = [128]
            for lTE = linspace(4.5e-3, 25e-3, 3)
            for lTR = linspace(50e-3, 4, 3)
            for lrxBW = 150*1e3
            for lconcatenations = 1:2:7
            for lnumSlices = [5 9]    
            ...
            acquisition.data.sliceThickness     = 6e-3;
            
            acquisition.data.fovSE              = 6e-3;
            acquisition.data.numSE              = 0;
            
            acquisition.data.concatenations     = lconcatenations;
            ...
            try
                ...
                if strcmpi(errorInfo{1}, 'userwarning')
                    ...
                    %% otherwise error
                    fprintf(1, '\n GRE CONC ERROR: %s (%d) : %s', ME.stack(1).file, ME.stack(1).line, ME.message);
                    fprintf(1, ['\n    PARAMETERS:',...
                        '  fovFE=%f  fovPE=%f  numFE=%d   numPE=%d',...
                        '  TE=%.3g  TR=%.3g  rxBW=%.2g',...
                        '  slices=%d  concatenations=%d'],...
                        lfovFE, lfovPE, lnumFE, lnumPE, lTE, lTR, lrxBW, lnumSlices, lconcatenations );
                    numFail = numFail + 1;
                end
            end
