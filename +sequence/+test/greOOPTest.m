...
% define the class
classdef greOOPTest < matlab.unittest.TestCase
    
    % parameterization of the tests
    properties (TestParameter)
        %% we will run sequential mode, so each column will be a test
        fovFE               = {0.15,        0.15,               0.23,               0.20,       0.23,       0.30,               0.30}; % field of view in m
        fovPE               = {0.15,        0.20,               0.23,               0.20,       0.20,       0.25,               0.30}; % field of view in m
        numFE               = { 128,        256,                127,                256,        256,        129,                256}; % freq encodings
        numPE               = { 128,        128,                127,                256,        255,        196,                128};  % phase encodings
        ...
        TR                  = {10e-3,       25e-3,              200e-3,             35e-3,      50e-3,      150e-3,             400e-3};
        TE                  = {3.5e-3,      10e-3,              4.5e-3,             15e-3,      25e-3,      5e-3,               7e-3};
        ...
        deltaOOP            = {3.5e-3,      4.5e-3,             2.24e-3,            6.0e-3,     4.0e-3,     4.5e-3,             3e-3};
        genB0map            = {0,           1,                  0,                  1,          0,          1,                  0};
    ...
    methods (Test, ParameterCombination = 'sequential')
        ...
        function runGreOOPEdgeTest(testCase,...
                fovFE,...
                fovPE,...
                numFE,...
                numPE,...
            ...
            % fixed vars
            acquisition.data.pulseSeqFamilyName = 'GRE-OOP';
            acquisition.data.pulseSeqActualName = 'GRE-OOP';
            ...
            acquisition.data.deltaTimeOOP = deltaOOP;
        ...
        function runGreOOPGeneralTest(testCase)
            ...
            for lfovFE = linspace(0.12, 0.45, 4)
            for lfovPE = linspace(0.15, 0.35, 3)
            for lnumFE = [96 127 256]
            for lnumPE = [64 127 192]
            for lTE = linspace(4.5e-3, 25e-3, 4)
            for lTR = linspace(50e-3, 4, 3)
            for lrxBW = [50 150 250]*1e3
            for lsliceThickness = 6*1e-3
            for lgenB0map = [0, 1]
            for ldeltaOOP = [2.24 4.5]*1e-3
            ...
            acquisition.data.fovSE              = lsliceThickness;
            ...
            acquisition.data.deltaTimeOOP = ldeltaOOP;
            ...
            try
                ...
                if strcmpi(errorInfo{1}, 'userwarning')
                    ...
                    %% otherwise error
                    fprintf(1, '\n GRE OOP ERROR: %s (%d) : %s', ME.stack(1).file, ME.stack(1).line, ME.message);
                    fprintf(1, ['\n    PARAMETERS:',...
                        '  fovFE=%f  fovPE=%f  numFE=%d   numPE=%d',...
                        '  TE=%.3g  TR=%.3g  rxBW=%.2g  sliceThicness=%.1g',...
                        '  deltaTimeOOP=%.3g   genB0=%d'],...
                        lfovFE, lfovPE, lnumFE, lnumPE, lTE, lTR, lrxBW, lsliceThickness, ldeltaOOP, lgenB0map);
                    numFail = numFail + 1;
                end
            end
