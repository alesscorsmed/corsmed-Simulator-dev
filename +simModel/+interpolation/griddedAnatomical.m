...
%
% SIMMODEL.INTERPOLATION.GRIDDEDANATOMICAL
%
...
%========================  CORSMED AB © 2020 ==============================
%
functionName = 'simModel:interpolation:griddedAnatomical';
% check args
if (nargin < 4)
...
if isempty(idxX) || isempty(idxY) || isempty (idxZ)
    ...
    % interpolate the indexes related to the tissue type
    v3d = reshape( 1:anatomicalModel.numVoxels, anatomicalModel.dimensions );
    ...
    if anatomicalModel.numFrames > 1
        ...
        % note: queryIdxFrame is 0 or 1 depending on if is part of the motion
        % note: origIdxFrame  is 0 if queryIdxFrame = 0, otherwise is the index in idxFrame
        [queryIdxFrame,origIdxFrame] = ismember(vq, anatomicalModel.idxFrame);
        ...
        model.tissueType(queryIdxFrame,:) = anatomicalModel.tissueTypeFrame(origIdxFrame(queryIdxFrame),:);
    else
        % non motion tissue
    ...
    if ~isempty( anatomicalModel.pdInhomogeneity )
        ...
        model.pd = []; % reshape(simodel.tissueValues(simModel.tissueType(:),3),[],1);
    end
