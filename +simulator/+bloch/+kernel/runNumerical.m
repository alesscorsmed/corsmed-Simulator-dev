function [solution] = runNumerical( solution, ...
    sequence, simModel, motionModel, expControl , GPUindex)
%
% SIMULATOR.BLOCH.KERNEL.RUNNUMERICAL
%
%	Runs an Numerical (for RF) Bloch Simulation, calling CUDA kernels,
%   using standard cartesian comps and ODE based time integration.
%
% INPUT
...
%========================  CORSMED AB © 2020 ==============================
%
functionName = 'simulator.bloch.kernel.runNumerical';
if (nargin < 4)
...
switch lower(expControl.simulation.odeMethod)
    case 'adaptiveexp'
        kernelFunction = 'adaptiveExplicitODE';
        rfMode = 'Adaptive Explicit';
    case 'implicit'
        kernelFunction = 'implicitODE';
        rfMode = 'Implicit';
    case 'adaptiveimp'
        kernelFunction = 'adaptiveImplicitODE';
        rfMode = 'Adaptive Implicit';
    otherwise
        kernelFunction = 'explicitODE';
        rfMode = 'Explicit';
end
% prepare the kernel
...
%% DF kernel -- default ANALYTICAL kernel, assume nothing
...
if  expControl.debug.debugMode
    fprintf(fid, '\nStaring Numerical simulation using Kernel File %s',kernelPtx);
    % for collecting data
    tRF = 0.0; nRF = 0;
    tGR = 0.0; nGR = 0;
    tRO = 0.0; nRO = 0;
    ...
    if expControl.debug.waitBarBE
        WB = waitbar(0, sprintf('Numerical Simulation: %d%%', 0 ), ...
    ...
    if  expControl.debug.debugMode
        ...
        if expControl.debug.waitBarBE
            ...
            waitbar(single(timeEnd)/single(numSteps), WB, ...
                sprintf('Numerical Simulation: %d%%', progress ));
        end
...
if  expControl.debug.debugMode
    ...
    fprintf(fid, '\n  RF part Time      %.3fs / %d steps (%.1fps/voxel/step) w/ %s',...
        tRF, nRF, 1e12*tRF/numIso/max(nRF,1), rfMode);
