function [solution] = runPhasor( solution, ...
    sequence, simModel, motionModel, expControl , GPUindex)
%
%
% SIMULATOR.BLOCH.KERNEL.RUNPHASOR
%
%	Runs a PHAse-ORiented Bloch Simulation, calling CUDA kernels.
%
% INPUT
...
%========================  CORSMED AB © 2020 ==============================
%
functionName = 'simulator.bloch.kernel.runPhasor';
if (nargin < 4)
...
if strcmpi(expControl.simulation.simulationEngine, 'ode')
...
%% Simplified RF kernel -- kernel for RF parts with no phase issues
kernelFunction = 'phasorExpSRF';
% prepare the kernel
KSF = parallel.gpu.CUDAKernel(kernelPtx, kernelProto, kernelFunction);
KSF.ThreadBlockSize = numThreads;
KSF.GridSize = numBlocks;
...
if  expControl.debug.debugMode
    ...
    if expControl.debug.waitBarBE
        WB = waitbar(0, sprintf('Phasor Simulation: %d%%', 0 ), ...
    ...
    if  expControl.debug.debugMode
        ...
        if expControl.debug.waitBarBE
            ...
            waitbar(single(timeEnd)/single(numSteps), WB, ...
                sprintf('Phasor Simulation: %d%%', progress ));
        end
        ...
        switch lower(partType)
            ...
            case 'sf'
                tSF = tSF + tkernel;
                nSF = nSF + single(timeEnd) - single(timeIni) + 1;
            case 'gr'
                tGR = tGR + tkernel;
...
if expControl.debug.debugMode
    ...
    fprintf(fid, '\n  SF part Time      %.3fs / %d steps (%.1fps/voxel/step)',...
        tSF, nSF, 1e12*tSF/numIso/max(nSF,1));
