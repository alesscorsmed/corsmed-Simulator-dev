...
writerObj = VideoWriter('diffusion_resolution1mm.avi');
...
for tt = 2:stride:length(t)-1
    ...
    if h > 0
        ...
        % Non-diffusion
        % relaxation
        [Mx, My, Mz] = numerical.numeric.relaxation(Mx, My, Mz, R1, R2, Moz, h);
        % precession
        [Mx, My] = numerical.numeric.precession(Mx, My, theta);
        
        % Diffusion
        dM = diffusion(Mx_df, n_x, n_y, n_z, d_x, d_y, d_z,...
                        Dxx, Dyy, Dzz, Dxy, Dyx, Dxz, Dzx, Dyz, Dzy);
        Mx_df = Mx_df + h*dM;
        dM = diffusion(My_df, n_x, n_y, n_z, d_x, d_y, d_z,...
                        Dxx, Dyy, Dzz, Dxy, Dyx, Dxz, Dzx, Dyz, Dzy);
        My_df = My_df + h*dM;
        ...
        Mz_df = Mz_df + h*dM;
        % relaxation
        [Mx_df, My_df, Mz_df] = numerical.numeric.relaxation(Mx_df, My_df, Mz_df, R1, R2, Moz, h);
        % precession
        [Mx_df, My_df] = numerical.numeric.precession(Mx_df, My_df, theta); 
...
title( sprintf('Diffusion effect at %.1f mm resolution\n time %.3f ms\n(No Diffusion) Signal = %f\n(W/ Diffusion) Signal = %f',...
       d_x*1e3, t(tt)*1e3, d_x*sum(abs(Mx(:) + 1j*My(:))), d_x*sum(abs(Mx_df(:) + 1j*My_df(:))) ) );
