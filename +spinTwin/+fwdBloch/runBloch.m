function [solution, stats] = runBloch( solution, ...
    sequence, simModel, motionModel, simControl, dbgControl )
%
% SPINTWIN.FWDBLOCH.RUNBLOCH
%
%	Runs basic Bloch simulation, calling CUDA kernels:
%       Choice of Numerical/Analytical for RF
%       Analytical exponential integration for remainder
%       Voxel point-matching integration
...
%========================  CORSMED AB © 2020 ==============================
%
functionName = 'spinTwin:fwdBloch:runBloch';
% check args
if (nargin < 5) ...
...
switch lower(simControl.precision)
    case 'double'
        kernelFunction = 'analyticalExpDFId';
    otherwise
        kernelFunction = 'analyticalExpDFIf';
end
% prepare the kernel
...
%% prepare info
if dbgControl.mode
    fprintf(fid, '\nStaring Bloch simulation using Kernel File %s',kernelPtx);
    fprintf(fid, '\n      0%%');
    progress = 0;
end
...
for ss = 1:sequence.numParts
    ...
    switch lower(sequence.partType{ss})
        ...
        otherwise % default
            stats.tDF = stats.tDF + tkernel;
            stats.nDF = stats.nDF + single(timeEnd) - single(timeIni) + 1;
    end
...
if  dbgControl.mode
    ...
    fprintf(fid, '\n  DF part Time      %.3fs / %d steps (%.1fps/voxel/step - %s)',...
        stats.tDF, stats.nDF, 1e12*stats.tDF/numIso/max(stats.nDF,1), simPrecision);
    fprintf(fid, '\n');
    if fid ~=1
        fclose(fid);
    end
