function [solution, stats] = runPhasorPlus( solution, ...
    sequence, simModel, motionModel, simControl, dbgControl )
%
% SPINTWIN.FWDBLOCH.RUNPHASORPLUS
%
%	Runs Phasor Bloch simulation, calling CUDA kernels:
...
%   Plus indicates that incorporates derivatives of Bi
%
% INPUT
...
%========================  CORSMED AB © 2020 ==============================
%
functionName = 'spinTwin:fwdBloch:runPhasorPlus';
% check args
if (nargin < 5) ...
...
try
    ...
    if ~isfield(simModel, 'dbix') || isempty(simModel.dbix)
        simModel.dbix = 0*simModel.pd;
    end
    if ~isfield(simModel, 'dbiy') || isempty(simModel.dbiy)
        simModel.dbiy = 0*simModel.pd;
    end
    if ~isfield(simModel, 'dbiz') || isempty(simModel.dbiz)
        simModel.dbiz = 0*simModel.pd;
    end
    ...
    dbix    = gpuArray(single(simModel.dbix(indexes)));
    dbiy    = gpuArray(single(simModel.dbiy(indexes)));
    dbiz    = gpuArray(single(simModel.dbiz(indexes)));
    % coil sens
...
kernelPtx  = simControl.ptxDEV; %% TODO: using dev kernel. Modified when tested
...
kernelProto = [ 'float *, float *, float *, ',... % Mx, My, Mz
                ...
                'const float *, const float *, const float *,' ... % dbix, dbiy, dbiz
...
switch lower(simControl.odeMethod)
    case 'analytical'
        kernelFunction = 'phasorPlusExpRF';
        rfMode = 'Analytical';
    otherwise
        % Explicit 
        kernelFunction = 'phasorPlusExplicitODE';
        rfMode = 'Explicit';
end
% define precision
...
kernelFunction = 'phasorPlusExpGR';
% define precision
switch lower(simControl.precision)
    case 'double'
        kernelFunction = sprintf('%sId', kernelFunction);
...
kernelFunction = 'phasorPlusExpRO';
switch lower(simControl.precision)
    case 'double'
        kernelFunction = sprintf('%sId', kernelFunction);
    otherwise
...
if dbgControl.mode
    fprintf(fid, '\nStaring Phasor Plus simulation using Kernel File %s',kernelPtx);
    fprintf(fid, '\n      0%%');
    progress = 0;
end
