function [solution, stats] = runGradients( solution, ...
    sequence, simModel, simControl, dbgControl )
%
% SPINTWIN.SBRBLOCH.RUNGRADIENTS
%
...
%                    (Output) Gradients: numIsochromats  x numRxCoils
...
%                    EMPTY: (Output) Jacobians: numIsochromats  x numRxs
...
%========================  CORSMED AB © 2020 ==============================
%
functionName = 'spinTwin:sbrBloch:runGradients';
% check args
if (nargin < 4) ...
...
try
    ...
    % allocate for local Gradients
    GPr = gpuArray(single(zeros(numIso,nCoils)));
    GPi = gpuArray(single(zeros(numIso,nCoils)));    
    GR1 = gpuArray(single(zeros(numIso,nCoils)));
    GR2 = gpuArray(single(zeros(numIso,nCoils)));
...
for ss = 1:sequence.numParts
    ...
    for cc = 1:nCoils
        ...
        Rx(cc,idxRx) = Srefx(cc,idxRx) - Sx(cc,idxRx);
        Ry(cc,idxRx) = Srefy(cc,idxRx) - Sy(cc,idxRx);
        % compute the contribution to the gradient ctranspose(J)*r 
        %   for the PD:
        %     JPr  =  JPDx + j*JPDy
        %     JPi  = -JPDy + j*JPDx
        ...
        %     | JPDx    -JPDy  | * | Pr | = | Sx |
        %     | JPDy     JPDx  |   | Pi |   | Sy |
        %   applying the transpose of the Jacobian to the residual, we get
        GPr(:,cc) = GPr(:,cc) + squeeze(JPDx(:,cc,1:localNumRx))*transpose(Rx(cc,idxRx));
        GPr(:,cc) = GPr(:,cc) + squeeze(JPDy(:,cc,1:localNumRx))*transpose(Ry(cc,idxRx));
        GPi(:,cc) = GPi(:,cc) - squeeze(JPDy(:,cc,1:localNumRx))*transpose(Rx(cc,idxRx));
        GPi(:,cc) = GPi(:,cc) + squeeze(JPDx(:,cc,1:localNumRx))*transpose(Ry(cc,idxRx));
        % for derivatives wrt R1
        GR1(:,cc) = GR1(:,cc) + squeeze(JR1x(:,cc,1:localNumRx))*transpose(Rx(cc,idxRx));
        GR1(:,cc) = GR1(:,cc) + squeeze(JR1y(:,cc,1:localNumRx))*transpose(Ry(cc,idxRx));
        % for derivatives wrt R2
        GR2(:,cc) = GR2(:,cc) + squeeze(JR2x(:,cc,1:localNumRx))*transpose(Rx(cc,idxRx));
        GR2(:,cc) = GR2(:,cc) + squeeze(JR2y(:,cc,1:localNumRx))*transpose(Ry(cc,idxRx));
    end   
	tGrads = toc(tGrads);
    % data collection
    stats.tGrads = stats.tGrads + tGrads;
...
solution.GPr = gather(GPr);
solution.GPi = gather(GPi);
solution.GR1 = gather(GR1);
solution.GR2 = gather(GR2);
...
if  dbgControl.mode
    ...
    fprintf(fid, '\n  Gradient Assembly %.3fs', stats.tGrads);
    fprintf(fid, '\n');
    if fid ~=1
        fclose(fid);
    end
