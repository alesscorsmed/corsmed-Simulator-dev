...
% % generate target model and sequence
% [pulseSequence,simModel,motionModel,mrSystem] = ...
%     spinTwin.test.sbr.utils.prepareExperimentDyn( fovX, fovY, fovZ, dx, dy, dz );

% generate target model and sequence
[pulseSequence,simModel,motionModel,mrSystem] = ...
    spinTwin.test.sbr.utils.prepareExperimentFISP( fovX, fovY, fovZ, dx, dy, dz );

spinTwin.test.sbr.utils.splot(pulseSequence)
...
optionsN = optimoptions('lsqnonlin', 'Display','iter', 'SpecifyObjectiveGradient',true, 'TolFun',1e-4,'TolX',1e-3,'MaxFunEval',1e3);
