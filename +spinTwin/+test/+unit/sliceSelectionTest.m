%
% SPINTWIN.TEST.UNIT.SLICESELECTIONTEST
%
...
% define the class
classdef sliceSelectionTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        %% test the the flip angle with slice selection
        % Compare with theoretical result (with loose accuracy)
        function theoreticalSliceSelection(testCase,...
                precision, odeMethod, engine, ...
                sliceThickness, rfDuration, rfFlipAngle, rfPhase, rfCycles )
            ...
            switch rfCycles
                case 10
                    ...
                    % magnitude reduction off slice is > 60%
                    offSliceReduction   = 0.40; 
                case 6
                    % within 5% of target FA in 90% of the slice thickness
                    allowedError        = 0.05; 
                    sliceSelectivity    = 0.85; 
                ...
                otherwise
                    ...
                    % magnitude reduction off slice is > 40%
                    offSliceReduction   = 0.60;
            end
            
            %% verify right slice selection
            idxInSlice = find(abs(simModel.z) < ...
                sliceSelectivity*sliceThickness/2);
            errorInSlice = abs(solSim.Mm(idxInSlice)-refMm(idxInSlice));
            testCase.verifyLessThan(errorInSlice,...
                allowedError*max(abs(refMm)));
            %% verify right off slice dampening
            idxOffSlice = find(abs(simModel.z) > ...
                sliceThickness/2);
            testCase.verifyLessThan(solSim.Mm(idxOffSlice),...
        ...
        function referenceSliceSelection(testCase,...
                precision, odeMethod, engine, ...
                sliceThickness, rfDuration, rfFlipAngle, rfPhase, rfCycles )
