...
def scatteredPython(tissue_ids,mySliceOnXY_model_spatial,xq,sliceLength,xqLength):
	...
	mySliceOnXY_model_spatial = np.reshape(np.transpose(np.array(mySliceOnXY_model_spatial)),[int(sliceLength),2])

	xq = np.reshape(np.transpose(np.array(xq)),[int(xqLength),2])
