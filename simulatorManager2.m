function simulatorManager2(redisAddress,redisPort,...
...
%     spinModelKey        = [uniqueID,'_spinModel'];
    ...
    %         expControlKey,simJobKey});
        ...
        % Get data from redis
        [pulseSequenceValue,~,~] = tools.redis.redisGet(R,pulseSequenceKey);
        pulseSequence = jsondecode(pulseSequenceValue{1,1});
        
        [motionModelValue,~,~] = tools.redis.redisGet(R,motionModelKey);
        motionModel = jsondecode(motionModelValue{1,1});
        ...
        if dummyData
            ...
            [simJobValue,~,~] = tools.redis.redisGet(R,simJobKey);
            simJob = jsondecode(simJobValue{1,1});
        end
    ...
    if useMatlabBuiltinRedisFunction
        ...
        [~, statusSet1] = tools.redis.redisSetJson(R,simSignalKey,...
                jsonencode(simSignal));
    end
